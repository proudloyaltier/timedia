...
function deleteTile(tile) {
  localStorage.files.replace("!!" + localStorgae.files.split("!!")[tile], "");
  save();
...
function loadTiles() {
  ...
  if (localStorage.files !== "") {
    ...
    for (var i = 0; i < localStorage.files.split(",").length; i++) {
      document.getElementById('tiles-tiles').innerHTML = document.getElementById('tiles-tiles').innerHTML + '<li style="float: left; width: 250px; height: 250px;" class="card" onclick="window.open(\'' + localStorage.files.split(",")[i].split("!!")[1] + '\');"><h3><center>' + localStorage.files.split(",")[i].split("!!")[0] + '<br><span style="font-size: 300%; color: #2296F3;" class="glyphicon glyphicon-th-large"></span><br><br><br><button class="btn btn-danger" onclick="deleteTile(' + i + ');">Delete</button></center></h3></span></li>';
    }
