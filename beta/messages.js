...
if (!localStorage.conversations == undefined || !localStorage.conversations == "") {
  ...
  for (var i = 0; i < conversations_split.length; i++) {
    ...
    document.getElementById('msgs').innerHTML = document.getElementById('msgs').innerHTML + '    <div id="chat' + conversations + '" class="tab-pane fade in"><h3>' + conversations_split[i] + '</h3><input type="text" id="input-chat' + conversations + '" class="form-control" onchange="sendMessage(\"' + [localStorage.name, conversations_split[i]].sort[0] + '>' + [localStorage.name, conversations_split[i]].sort[0] + '"\, this.value)" placeholder="Send a message..."></div>';
